...
*   **📋 章节细纲功能**
    *   在章节大纲和章节正文之间增加中间层，提供场景分解、关键对话、人物情绪分析等详细情节分解
    *   支持人物情绪追踪，记录角色在章节中的情绪变化

*   **🎭 智能角色卡更新**
    *   章节正文保存时自动提取角色信息
    *   自动更新或创建角色卡，记录角色最近3个章节的状态
    *   增强的人物塑造字段：致命缺陷、性格矛盾、隐藏秘密、说话风格

*   **📚 去AI味写作支持**
    *   内置去AI味写作指南和自然中文写作指引
    *   AI生成内容时自动引用知识库，生成更自然的内容
...
- **新增章节细纲功能**：在章节大纲和章节正文之间增加中间层，提供更详细的情节分解
  - 包含场景分解、关键对话、人物情绪分析、情节转折、角色成长、伏笔铺垫等字段
  - 人物情绪字段支持记录角色名称、情绪类型、强度、触发事件和转变过程
  
- **增强角色卡模型**：新增多个字段提升人物塑造能力
  - 致命缺陷/弱点（flaw）
  - 性格矛盾/反差（contradiction）
  - 隐藏秘密（secret）
  - 说话风格/口头禅（speaking_style）
  - 最近章节记录（recent_chapters）：自动记录角色最近3个章节的状态
  
- **新增知识库**：
  - 去AI味写作指南：帮助生成更自然、更具"人味"的内容
  - 自然中文写作指引：包含语言细节清单、去AI味润色流程、展示而非讲述原则
  - 人物塑造指南：包含动机链分析、三层标签反差法、反派层级设计、人物成长弧线
  
- **新增工作流**：
  - 章节细纲生成工作流：章节大纲保存时自动创建章节细纲卡片
  - 角色卡自动更新工作流：章节正文保存时自动提取角色信息，更新或创建角色卡
  
- **优化提示词模板**：内容生成和章节细纲提示词自动引用去AI味写作指南、自然中文写作指引和人物塑造指南
...
**3. 前端 (Web模式)**
如果你不想使用Electron，也可以通过纯Web方式运行前端：
...
- **章节细纲生成**: 章节大纲保存时自动创建章节细纲卡片
- **角色卡自动更新**: 章节正文保存时自动提取角色信息并更新角色卡
